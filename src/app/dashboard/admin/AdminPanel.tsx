      ...
      <header className="h-28 border-b border-white/5 sticky top-0 z-50 px-8 flex items-center justify-between bg-black/60 backdrop-blur-2xl">
        <div className="flex items-center gap-12">
          <div className="relative group perspective-1000">
            <Link href="/dashboard" className="w-16 h-16 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center transform group-hover:rotate-12 transition-all duration-500 hover:border-jarvis-accent/50 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
              <Zap className="text-jarvis-accent w-8 h-8 drop-shadow-[0_0:10px_rgba(34,211,238,0.8)]" />
            </Link>
        ...
        <div className="hidden lg:flex items-center gap-12">
           <div className="relative w-96 group">
             ...
             <input 
               ...
               className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-14 pr-6 text-[10px] font-black tracking-[0.2em] outline-none focus:border-jarvis-accent/30 focus:bg-white/[0.08] transition-all text-white placeholder:text-white/10 font-mono"
             />
           </div>
           ...
           <div className="flex items-center gap-5 font-sans">
              <div className="text-right">
                ...
                <button onClick={() => setIsEditingName(true)} className="text-lg font-black text-white hover:text-jarvis-accent transition-colors uppercase tracking-tight flex items-center gap-3">
                  {adminName}
           ...
           <div className="jarvis-box !p-10 backdrop-blur-3xl overflow-hidden">
              ...
              <div className="relative space-y-8 max-w-4xl">
                 <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-[12px] font-black uppercase tracking-[0.5em] text-jarvis-accent shadow-[0_0_10px_rgba(34,211,238,0.3)]">BUDDY_OS INTEL</h2>
                 </div>
             ...
             <div key={i} className="group relative">
                ...
                <div className="jarvis-box !p-10 backdrop-blur-3xl transform transition-all duration-500 hover:-translate-y-4 hover:rotate-2 hover:shadow-[0_40px_80px_rgba(0,0,0,0.6)] group-hover:border-jarvis-accent/30">
                   <div className="flex justify-between items-start mb-8">
                      ...
                      <div className="text-[10px] font-black text-jarvis-accent/40 uppercase tracking-[0.2em]">{stat.accent}.lvl1</div>
                   </div>
                   <div className="space-y-2">
                      <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-white/30">{stat.label}</h4>
                      <p className="text-6xl font-black text-white tracking-tighter group-hover:text-jarvis-accent transition-colors font-mono">{stat.val}</p>
                   </div>
              ...
              <div className="relative group">
                 ...
                 <div className="jarvis-box !p-10 !rounded-[50px] backdrop-blur-md shadow-2xl group-hover:border-jarvis-accent/20 transition-all">
                       ...
                       {recentLogins.map((user, idx) => (
                         <div key={user.id} className="group/log relative p-5 bg-white/[0.02] border border-white/5 rounded-[25px] hover:bg-white/5 hover:border-jarvis-accent/20 transition-all duration-300">
                            <div className="flex items-center gap-5">
                               <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center font-black text-white/20 group-hover/log:text-jarvis-accent transition-colors">
                               ...
                               <div className="text-right">
                                  <p className="text-[10px] font-black text-jarvis-accent mb-0.5 font-mono">{new Date(user.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5 flex items-center justify-between">
                       ...
                       <button className="text-[10px] font-black text-jarvis-accent hover:text-white transition-colors uppercase tracking-widest">Refine Matrix</button>
                    </div>
                 </div>
              </div>
           </div>
                    ...
                    <form onSubmit={handleAddUser} className="relative bg-white/[0.03] border border-jarvis-accent/20 p-10 rounded-[40px] backdrop-blur-2xl flex flex-col md:flex-row gap-6 shadow-2xl">
                       <div className="flex-1 relative">
                          ...
                          <input 
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-16 pr-8 text-[11px] font-black text-white tracking-[0.2em] outline-none focus:border-jarvis-accent focus:bg-white/10 transition-all font-mono"
                            required
                          />
                       </div>
                       <button type="submit" className="bg-jarvis-accent text-black px-12 py-5 rounded-2xl font-black text-[12px] uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-jarvis-accent/20 active:scale-95">
                          Authorize Status
                       </button>
                    </form>
                 </div>
                       ...
                       <table className="w-full text-left border-collapse">
                          <thead>
                             <tr className="bg-white/[0.08] border-b border-white/10">
                                <th className="px-12 py-10 text-[11px] font-black uppercase tracking-[0.4em] text-jarvis-accent border-r border-white/5">
                                ...
                                <th className="px-12 py-10 text-[11px] font-black uppercase tracking-[0.4em] text-jarvis-accent text-right">
                               ...
                               <tr key={s.id} className="group/row hover:bg-white/[0.03] transition-all">
                                  <td className="px-12 py-10">
                                     <div className="flex items-center gap-6">
                                        <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center font-black text-white/20 group-hover/row:scale-110 group-hover/row:border-jarvis-accent/40 group-hover/row:text-jarvis-accent transition-all font-mono">
                                           {s.email[0].toUpperCase()}
                                        </div>
                                        <div className="space-y-1.5">
                                           <p className="text-lg font-black text-white tracking-tight group-hover/row:text-jarvis-accent transition-colors font-mono">{s.email}</p>
                    ...
                    <div className="px-12 py-10 bg-white/[0.02] border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-10">
                       <div className="flex items-center gap-8">
                          <div className="flex -space-x-4">
                             {[...Array(5)].map((_, i) => (
                               <div key={i} className="w-12 h-12 rounded-full border-4 border-black bg-white/5 flex items-center justify-center font-black text-xs text-white/10 font-mono">
                                  {i + 1}
                               </div>
                             ))}
                          </div>
